...
def create_dataset(datasets: list, datasetNames: list, config):
    ...
    for dataset, name in zip(datasets, datasetNames): #  no need for for loop
        ...
        for split in dataset:
            ...
            dt = dt.rename_column("English", "labels")   # output    lug_eng
...
model = AutoModelForSeq2SeqLM.from_pretrained("Helsinki-NLP/opus-mt-mul-en") #  loads the model
tokenizer = AutoTokenizer.from_pretrained("Helsinki-NLP/opus-mt-mul-en") # loads tokenizer
