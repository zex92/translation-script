...
def create_dataset(datasets: list, datasetNames: list, config):
    ...
    for dataset, name in zip(datasets, datasetNames): #  no need for for loop
        ...
        for split in dataset:
            ...
            dt = dt.rename_column("Luganda", "labels")   # ioutput
            dt = dt.rename_column("English", "input_ids")  #input
...
model = AutoModelForSeq2SeqLM.from_pretrained("Helsinki-NLP/opus-mt-en-mul") #  loads the model
tokenizer = AutoTokenizer.from_pretrained("Helsinki-NLP/opus-mt-en-mul") # loads tokenizer
